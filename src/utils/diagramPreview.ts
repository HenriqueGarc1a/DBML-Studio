...
export async function captureDiagramPreview(svg: SVGSVGElement | null, tables: TableModel[]): Promise<string | undefined> {
  ...
  clone.querySelectorAll(".relation-edit-handles, .relation-endpoint-handles, .relation-segment-hitbox, .relation-edit-feedback, .relation-snap-guides, .table-route-obstacle-indicator, .resize-handle, .relation-flow-arrow").forEach((node) => node.remove());
