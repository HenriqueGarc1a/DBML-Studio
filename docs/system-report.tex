...
\texttt{src/renderer/SvgCanvas.tsx} & Canvas SVG, pan/zoom, toolbar flutuante, drag de tabelas/grupos, criação visual de relações e manipulação de pontos. \\
...
A entrada visual fica em \texttt{src/App.tsx}, no botão \texttt{SQL} do header. O modal coleta o SQL e chama \texttt{controller.createDiagramFromSql}. O controlador usa \texttt{sqlToDbml} em \texttt{src/importer/sqlToDbml.ts}, cria um novo registro de diagrama e reaproveita o fluxo normal de importação DBML para parsear, aplicar layout e salvar.
...
As relações vivem em \texttt{RelationModel}. A geometria básica é calculada em \texttt{src/utils/geometry.ts}; a distribuição de endpoints compartilhados fica em \texttt{src/utils/relationLayout.ts}; o roteamento de organização que desvia de tabelas fica em \texttt{src/utils/relationRouting.ts}; e o desenho final é feito por \texttt{RelationPath.tsx}. A toolbar flutuante em \texttt{SvgCanvas.tsx} tem a ação \texttt{Organizar relações}, que chama \texttt{tidyRelations}. No painel de propriedades, \texttt{Auto rota} chama \texttt{tidyRelation}. O roteador procura primeiro as laterais esquerda/direita das tabelas, testa combinações de lados e escolhe uma rota ortogonal curta que não cruza as tabelas.

Para melhorar o estilo das linhas, comece por \texttt{RelationPath.tsx}. Para mudar regras de rota automática que evitam tabelas, mexa em \texttt{relationRouting.ts}. Para mudar comandos de limpeza/autoajuste, mexa em \texttt{tidyRelationGeometry} dentro de \texttt{useDiagramController.ts}.
